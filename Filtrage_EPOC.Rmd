...
## Processus de filtrage du jeu de donnees EPOC (<u>02_filtrage_epoc.R</u>)
  ...
  Ce document montre les differents filtres applique aux données issu de faune-france afin d'obtenir des observations issues du protocole EPOC. Ainsi que le mise en place d'indicateur (Diversite par liste et Abondance par liste) permettant d'évaluer la qualité des observations.

* Les différents filtres appliqués sont :
    + Filtrage selon le mentionnement d'EPOC dans les commentaires / remarques des listes d'observations
    ...
    + Selection des listes complètes
    ...
    + Formation de 2 tableaux (un format court et un format long) et filtrage selon la periode (1/03 - 30/06)

  Chaque étape de filtrage du script est enregistrée sur le disque dur.

### Filtrage selon le mentionnement d'EPOC dans les commentaires / remarques des listes d'observations

  Certaines listes mentionnés le protocole EPOC dans les remarques d'observations et non dans les commentaires de la liste. Cela a donc necessite d'uniformise le mentionnement d'EPOC dans les listes où il était inscrit dans les commentaires.
...
```
  ...
  Après le 3eme filtre, ont obtiens le même nombre d'observations. Le jeu de données ne contenait pas d'erreur de saisie.
  
### Retrait des observations avec comme "Type de localisation" : jardin
  Les observations avec la mention de jardin dans le type de localisation ont été retiré du jeu de données, car celles-ci pourrait être une source de confusion entre le protocole "Oiseaux des jardins" et la réalisation d'une EPOC dans un jardin par un observateur.
...
```
  800 observations ont été retiré du jeu de données. Ce qui signifie que la grande majorité des observations EPOC renseigné sur la totalite des oiseaux renseigné.
...
### Retrait des observations avec comme détails "en main" et "analyse de pelotes"
  Ce filtrage a necessité du travail sur la varaible "Details" qui regroupe des informations (Nombre/sexe/condition de contact) lors des observations. Les conditions d'observations que j'ai retiré sont "en main" et "analyse de pelotes".
  J'ai decidé de "segmenté" les informations de la colonne Details et de crée 4 colonnes (Nb_pose / Nb_vol / Nb_audition / Nb_NA) afin de récupérer ces informations triées.
